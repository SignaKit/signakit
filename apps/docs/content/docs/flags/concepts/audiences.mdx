...
```ts title="Node.js"
import { createClient } from '@signakit/flags-node';

const signakit = createClient({ apiKey: process.env.SIGNAKIT_API_KEY });

// Attributes are matched against audience rules at evaluation time.
const user = signakit.createUserContext('user_abc123', {
  plan: 'beta',
  role: 'member',
  country: 'US',
  accountAgeDays: 42,
...
const enabled = await user.decide('new-dashboard');
